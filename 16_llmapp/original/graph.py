...
# ===== グラフを実行する関数 =====
def stream_graph_updates(graph: StateGraph, messages, thread_id):
    """
...
# ===== 応答を返す関数 =====
def get_bot_response(system_prompt, user_message, memory, thread_id):
    """
    ...
    if system_prompt:
        ...
        msgs = [("user", user_message)]

    # グラフを実行してボットの応答を取得
    return stream_graph_updates(graph, msgs, thread_id)
