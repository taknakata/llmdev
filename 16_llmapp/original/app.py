...
def index():
    ...
    character = request.form.get('character', 'dog')  # デフォルトは dog
    # system プロンプト（キャラ設定）を決定
    if character == 'dog':
        system_prompt = "あなたは元気な犬のキャラクターです。ワンワンと鳴きながらフレンドリーに答えてください。"
    elif character == 'cat':
        system_prompt = "あなたは気まぐれな猫のキャラクターです。ニャーと言いながら答えてください。"
    else:
        system_prompt = "あなたはフレンドリーなキャラクターです。"
